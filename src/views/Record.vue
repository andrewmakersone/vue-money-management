<template>
  <div>
    ...
    <p v-else-if="!categories.length" class="center">Катеорий пока нет. <router-link to="/categories">Добавить новую категорию.</router-link></p>
      ...
      async submitHandler() {
        ...
        if (this.canCreateRecord) {
          try {
            ...
            this.$quickmessage('Запись успешно созданаэ');
            this.$v.$reset();
            this.amount = 1;
            this.description = ''
          } catch (e) { return e }
          ...
          this.$quickmessage(`Недостаточно средств на счете (${this.amount - this.info.bill})`)
        }
      }
    },
    async mounted() {
